...
function rechteckGenerieren(): void {
    let _rechteck: HTMLDivElement = document.createElement("div");
...
function rechteckEntfernen(): void {
    let _divlänge: number = document.getElementsByTagName("div").length;

    for (let i: number = 0; _divlänge > i; i++) {
        let _rechteckarray: HTMLElement = document.getElementById("rechteck" + i);
...
namespace Tischpflanze {
    ...
    //Buttons
    function createMenuItem(name: string): HTMLButtonElement {
    ...
    let indexzähler: number = 0;
    function createImages (_path: string, _idname: string): HTMLImageElement {
    ...
    let pflanzenauswahl: Pflanze = null;
    ...
    function bilder(_event: Event): void {
        ...
        pflanzenauswahl = pflanze[imagenumber];
